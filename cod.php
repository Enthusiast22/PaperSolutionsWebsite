...
$result3=select("SELECT items.sol
FROM cart
INNER JOIN items ON cart.itemid = items.itemid WHERE userid='".$_SESSION['userid']."'");
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="breadcumb-area bg-img bg-overlay" style="background-image: url(img/bg.png);"></div>
	</br>
...
<h4 class="container"> Thank you for Ordering at PAPER SOLUTIONS'! The ordering process is now complete.</h4>
...
<?php while($r=mysqli_fetch_array($result3))
					 {
						 extract($r);
					 ?>
     <a href="download.php?file=<?php  echo "$sol"; }?>"  class="btn btn-primary">Download Solution</a>
